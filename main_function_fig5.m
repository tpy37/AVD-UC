...
function [omega_n_store, max_velocity_store, drag_store] = main_function_fig5(p_a,acoustic_f, rho_0, v_sound, visc, r_min, r_max, r_incr, rho_min, rho_max, rho_incr, save_data)
...
grav_buoy_store = zeros(rho_incr, r_incr); % Stores the gravitation force at the point
...
for rh = 1:length(rho_screen) % Sweep in rho
    ...
    parfor ii = 1:length(r) % Sweep in radius
        ...
        %% Record ARF and Grav data
        arf_store(rh, ii) = abs(alpha_c(F2,p_a,rho_0,acoustic_f));  %#ok<PFOUS>
        grav_buoy_store(rh, ii) = F_gravity_buoyancy(r(ii),rho_p,rho_0);  %#ok<PFOUS>
        
        %% Setup functions for Newton Raphson Optimization
        function_t = @(v_cand) abs(alpha_c(F2,p_a,rho_0,acoustic_f))+Drag_Force_Zero_Escape(1, r(ii),rho_0,v_cand,visc)+ F_gravity_buoyancy(r(ii),rho_p,rho_0);
        ...
        max_alpha_gravity_buoy = abs(alpha_c(F2,p_a,rho_0,acoustic_f))+ F_gravity_buoyancy(r(ii),rho_p,rho_0);
        Mass_total = mass(r(ii),rho_p)+virtual_mass(r(ii),rho_0); % inertia needs to consider both particle and added mass
        omega_n_max = max_alpha_gravity_buoy./(Mass_total*v_cand);     
...
elseif max(max(max_velocity_store)) >= v_sound
    ...
    if save_data==1
        ...
        if status
            save(['results/study_ff_' num2str(acoustic_f) '_Hz_pp_' num2str(p_a) '_Pa.mat'], 'A_pix', 'omega_n_store',...
                'max_velocity_store','drag_store','arf_store','grav_buoy_store',...
